...
public class EslEvent {
    ...
    public EslEvent(EslMessage rawMessage, boolean parseCommandReply) {
        ...
        // plain or xml body
        if (Value.TEXT_EVENT_JSON.equals(rawMessage.getContentType())) {
            parsePlainBody(rawMessage.getBodyLines());
            eventHeaders = rawMessage.getEventHeader();
        } else if (Value.TEXT_EVENT_PLAIN.equals(rawMessage.getContentType())) {
            parsePlainBody(rawMessage.getBodyLines());
        } else if (Value.TEXT_EVENT_XML.equals(rawMessage.getContentType())) {
            throw new IllegalStateException("XML events are not yet supported");
        } else if (Value.COMMAND_REPLY.equals(rawMessage.getContentType()) && parseCommandReply) {
            parsePlainBody(rawMessage.getBodyLines());
        } else if (Value.LOG_DATA.equals(rawMessage.getContentType())) {
            parsePlainBody(rawMessage.getBodyLines());
        } else {

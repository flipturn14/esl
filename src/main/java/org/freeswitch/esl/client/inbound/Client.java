...
public class Client {
    ...
    private final Executor                otherMessageListenerExecutor  = Executors.newSingleThreadExecutor(
            new ThreadFactory() {
                AtomicInteger threadNumber = new AtomicInteger(1);

                public Thread newThread(Runnable r) {
                    return new Thread(r, "EslOtherMessageNotifier-" + threadNumber.getAndIncrement());
                }
            });
...
//        }
     ...
     * @return a {@link CommandResponse} with the server's response.
     */
    public CommandResponse setNixEvent(String format, String events) {
        // temporary hack
//        if (!format.equals("plain")) {
//            throw new IllegalStateException("Only 'plain' event format is supported at present");
//        }
    ...
    private final IEslProtocolListener protocolListener = new IEslProtocolListener() {
        ...
        public void otherMessageReceived(EslEvent eslEvent) {
            for (final IEslEventListener listener : eventListeners) {
                otherMessageListenerExecutor.execute(new Runnable() {
                    public void run() {
                        try {
                            listener.otherEventReceived(eslEvent);
                        } catch (Throwable t) {
                            log.error("Error caught notifying listener of job result [" + eslEvent + ']', t);
                        }
                    }
                });
            }
